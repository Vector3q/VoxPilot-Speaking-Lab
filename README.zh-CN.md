...
- Start Easy 起步训练：4 步本地脚手架，不强制使用 AI
- 11 题完整练习模式
- 110 道仿真题：60 道复述题，50 道问答题
- 浏览器录音、回放和语音识别转写
- 1-6 半分制本地模拟评分
- 音频质量、停顿、语速、信噪比、本地可理解度代理和评分可信度诊断
- Basic 模式下的疑似音素训练点，以及可选外部逐音素评分
- 同题复练对比
- 历史记录和弱项追踪
- 发音错题本：自动聚合高频音素训练点并支持回顾
- 本地数据管理和隐藏 API key 的完整导出
- 自适应 Ladder 训练：诊断分级、今日任务、阶梯升级
- Coach Capsule：诊断、micro-drill、重答和 before / after 复盘
- API Setup 页面区分 Basic 本地功能、AI 评分、AI Coach 和外部发音评分
...
`Start Easy` 的 Step 1-3 使用更轻量的本地检查，只显示 Pass、Almost 或 Try Again。Step 4 才回到完整本地评分系统。这个模式不会强制调用 AI 内容评分或外部发音评分。

每次评分后都会显示“评分可信度”。它会解释这次反馈是否有足够的转写长度、录音时长、录音质量、浏览器 ASR 置信度，以及可选外部评分信号支撑。
...
Basic 模式下，发音部分是“本地可理解度代理”，不是严格逐音素评分。如果配置了外部发音 API，系统会把外部返回的 phoneme score 单独展示，并优先作为逐音素参考。
...
- 导出全部本地数据时会隐藏 API key，只保留是否填写过 key
